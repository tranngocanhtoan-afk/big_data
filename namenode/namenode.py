            ...
            try:
                ...
                with lock:
                    if typ == 'register':
                        ...
                        node_id= msg.get("id")
                        print(f'receive task_complete message from {block_id}')
                        ...
                        print(f"[NameNode] receive node_free message from {node_id}")
                        file_base = msg.get('file')
...
def handle_task_completion(node_id: str, block_id: str, role: str = 'leader'):
    ...
    - node_id: node vừa hoàn thành task (leader)
    - block_id: block đã hoàn thành
    - role: 'leader' hoặc 'storage' (default 'leader')
    """
    import psycopg2
        ...
        with conn_meta.cursor() as cur:
            ...
            try:
                with conn_file.cursor() as cur_file:
                    ...
                    # Lấy followers từ bảng block của file này
                    cur_file.execute(
                        sql.SQL("SELECT followers FROM {} WHERE block_id = %s").format(tbl),
                        (block_id,)
                    )
                    result = cur_file.fetchone()
                    # Nếu có followers thì lấy ra (giả sử là list), nếu không thì []
                    followers = result[0] if result and result[0] else []
                    
                    # Update block status thành completed, leader/followers = NULL
                    cur_file.execute(
                        sql.SQL("""
            ...
            # Nếu followers là string kiểu '["nodeA","nodeB"]' thì chuyển về list
            ...
            # Reset storage cho từng follower (xóa block khỏi storage)
            for follower in followers:
                ...
                cur.execute(
                    ...
                    """,
                    (
                        block_id,                  # 1. storage == block_id
                        f"{block_id},%",           # 2. block_id ở đầu
                        f"{block_id},",
                        f"%,{block_id}",           # 3. block_id ở cuối
                        f",{block_id}",
                        f"%,{block_id},%",         # 4. block_id ở giữa
                        f",{block_id},",
                        follower                   # 5. node_id follower cần update
                    )
                )
            ...
            # TODO: Gán task mới cho node_id này nếu có task mới chờ (nếu cần)
            # assign_new_task_if_available(node_id)  # Tùy hệ thống bạn có hàm này không
...
def handle_node_free(node_id: str, file_base: str):
    ...
    try :
        with conn_meta.cursor() as cur:
            ...
            print(f"[NameNode] Node {node_id} is set freeeeeeeeeeeeeeeeeeeeeee")
            conn_meta.commit()
