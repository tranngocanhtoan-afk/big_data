    ...
    try:
        with conn_meta.cursor() as cur:
            if role == 'leader':
                ...
                # For storage tasks, just remove from storage
                cur.execute(
                    """
                    UPDATE active_node_manager SET
                      storage = CASE
                     ...
                     WHERE node_id = %s;
                    """, 
                    (block_id, f"{block_id},%", f"{block_id},", f"%,{block_id}", f",{block_id}", node_id)
                )
                print(f"[NameNode] Storage task {block_id} completed by {node_id}")
